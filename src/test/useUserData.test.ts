...
import { useUserData, getLevelTitle, getXPForLevel } from '../hooks/useUserData';
...
describe('useUserData — сохранение результатов', () => {
  it('saveResult добавляет запись в history', () => {
    ...
    act(() => {
      result.current.saveResult({ type: 'reaction', score: 245 });
    });
  ...
  it('saveResult присваивает id и timestamp', () => {
    ...
    act(() => {
      result.current.saveResult({ type: 'stroop', score: 88 });
    });
  ...
  it('saveResult сохраняет в localStorage', () => {
    ...
    act(() => {
      result.current.saveResult({ type: 'memory', score: 7 });
    });
  ...
  it('новые результаты добавляются в начало списка', () => {
    ...
    act(() => {
      result.current.saveResult({ type: 'reaction', score: 200 });
    });
    act(() => {
      result.current.saveResult({ type: 'stroop', score: 90 });
    });
