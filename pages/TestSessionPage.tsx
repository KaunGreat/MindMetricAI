...
const TestSessionPage: React.FC = () => {
  ...
  const handleComplete = useCallback((score: number | string, metadata?: any) => {
    // Save the data using the new saveResult function
    saveResult({
      type: testId || 'unknown-test',
      score: score,
      details: metadata
    });
    ...
    // Short delay before redirecting to profile as requested by the flow logic
    setTimeout(() => {
      navigate('/profile');
    }, 1500);
  ...
  const renderTest = () => {
    switch (testId) {
      case 'reaction':
        return <ReactionTest onComplete={handleComplete} onCancel={handleCancel} />;
      case 'spatial-span':
        return <SpatialSpanTest onComplete={handleComplete} onCancel={handleCancel} />;
      case 'memory':
        return <MemoryTest onComplete={handleComplete} onCancel={handleCancel} />;
      case 'stroop':
        return <StroopTest onComplete={handleComplete} onCancel={handleCancel} />;
      case 'wisconsin':
        return <WisconsinTest onComplete={handleComplete} onCancel={handleCancel} />;
      case 'pal':
        return <PALTest onComplete={handleComplete} onCancel={handleCancel} />;
      case 'nback':
        return <NBackTest onComplete={handleComplete} onCancel={handleCancel} />;
      case 'visual-search':
        return <VisualSearchTest onComplete={handleComplete} onCancel={handleCancel} />;
      case 'tower':
        return <TowerOfLondonTest onComplete={handleComplete} onCancel={handleCancel} />;
      case 'gambling':
        return <GamblingTaskTest onComplete={handleComplete} onCancel={handleCancel} />;
      case 'emotion':
        return <EmotionRecognitionTest onComplete={handleComplete} onCancel={handleCancel} />;
      case 'digit-span':
        return <DigitSpanTest onComplete={handleComplete} onCancel={handleCancel} />;
      case 'gonogo':
        return <GoNoGoTest onComplete={handleComplete} onCancel={handleCancel} />;
      default:
